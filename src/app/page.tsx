...
function Section({
  ...
  return (
    <section>
      <h2 className="text-xl font-semibold tracking-tight text-neutral-900 mb-4">{title}</h2>
      <ul className="space-y-3">
        {items.map((item) => (
          <li key={item.text}>
            <div className={`flex items-center gap-2 text-neutral-900 ${item.bold ? "font-medium" : ""}`}>
              <span className="inline-block w-1.5 h-1.5 rounded-full shrink-0 bg-blue-500" />
              {item.text}
            </div>
            {item.desc && (
